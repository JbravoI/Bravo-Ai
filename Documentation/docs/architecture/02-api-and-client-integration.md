...
| `/api/query` | POST | Gemini 3.6 Flash + MongoDB `qa_log` | Authenticated; grounded in tracked regulations; rate-limited and audited |
...
2. **Secrets never reach a Route Handler's response or a client-side `fetch`.** The Gemini API key lives in the server-only `GEMINI_API_KEY` env var, read only inside `app/src/app/api/query/route.ts`. Nothing under `app/src/components/` or client-side code should ever hold a provider API key.
...
`app/src/components/QAPanel.tsx` posts `{ question: string }` to the app's own `/api/query` and expects `{ answer: string }` on success or `{ error: string }` on failure. The server authenticates the request, supplies the fixed system instruction and tracked-regulation context to Gemini 3.6 Flash, and writes the completed exchange to `qa_log`. Citation text is requested from the model; structured citation fields remain a future enhancement.
